...
const TodoList: FC<TodoListProps> = ({ user, setLoginUser }: TodoListProps) => {
  ...
  const handleSendMessage = (
    ...
    return todoService
      .sendMessage(phone, dialogId, message, participants)
      .then((res) => {
            ...
            <div className={s.dialogues}>
              {loadingDialogs ? (
                ...
                dialogs.map((dialog) => {
                  ...
                  const imageUrl = `http://localhost:8081/channels/${formatedChannel}.png`;
                  const phone =
                    ...
                    {showParticipants && (
                      <div className={s.participantsTable}>
                        {participants.map((participant) => (
                          ...
                          >
                            <img
                              src={`http://localhost:8081/participants/${participant.username}.png`}
                              alt="Profile"
                              className={`${s.image} ${
                                expandedParticipant === participant.id
                                  ? s.imageExpanded
